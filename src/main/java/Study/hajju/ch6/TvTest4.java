...
public class TvTest4 {
    ...
    정확한 용어로는 "멤버 접근 연산자"라고 합니다.
    이 연산자는 객체의 속성이나 메소드에 접근할 때 사용됩니다.

    멤버 접근 연산자 (.)
    멤버 접근 연산자는 객체의 멤버(필드, 메소드, 내부 클래스 등)에 접근할 때 사용되는 연산자
    객체 지향 프로그래밍에서 객체의 상태는 필드(또는 속성)에 의해, 행동은 메소드에 의해 정의됩니다.
    따라서, 객체의 특정 필드 값을 읽거나 수정하거나, 객체가 가진 메소드를 호출할 때 멤버 접근 연산자를 사용합니다.
    객체의 멤버에 접근하려면 객체 이름 뒤에 점(.)을 붙이고 그 뒤에 멤버의 이름을 작성합니다.

    정리
    멤버 접근 연산자는 객체의 멤버(필드, 메소드 등)에 접근하기 위해 사용되는 연산자입니다.
